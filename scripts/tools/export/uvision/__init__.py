...
class Uvision(Exporter):
    ...
    def generate(self):
        ...
        if self.resources.linker_script is None:
            self.resources.linker_script = os.getcwd()+"\zephyr\linker.cmd"

        sct_file = self.resources.linker_script
        ctx['linker_script'] = sct_file
        print("linker:"+self.resources.linker_script)
        #print("VARUN:" + str(ctx['project_files']))
