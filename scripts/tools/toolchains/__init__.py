...
import sys
import json
from os import stat, walk, getcwd, sep, remove, system
...
class mbedToolchain:
    ...
    MBED_CONFIG_FILE_NAME="config.h"
    ...
    def scan_resources_zephyr(self, path, exclude_paths=None, base_path=None,
                       collect_ignores=False):

        resources = Resources(path, collect_ignores=collect_ignores)
        #print('VARUN:resource:' + str(resources))

        if not base_path:
                base_path = getcwd()
        resources.base_path = base_path

        print("scan:", path, base_path)
        system("ninja -t deps > deps_files.txt")
        with open('deps_files.txt') as fileobj:
             for path in fileobj:
                 self._add_file(abspath(path.strip()), resources, base_path, exclude_paths=exclude_paths)
                #self._add_file(path.strip(), resources, base_path, exclude_paths=exclude_paths)
        return resources
    ...
    def _add_file(self, file_path, resources, base_path, exclude_paths=None):
        ...
        #print("resources.file_basepath[file_path]:", base_path)
        ...
        if   ext == '.s':
            ...
            #print("resources.file_basepath[file_path]:", file_path)
